...
describe('Contrived problems (clean data)', () => {
  ...
  contrivedProblems.forEach((problem) => {
    ...
    testInvocation(`Should fit ${problem.name}`, () => {
      const { getFunctionFromParameters, n, xStart, xEnd, problemParameters, options, decimalsForParameterError, decimalsForParameterValues } = Object.assign({
      ...
      expect(actual.parameterValues).toBeDeepCloseTo(problemParameters, decimalsForParameterValues);
      expect(actual.parameterError).toBeDeepCloseTo(0, decimalsForParameterError);
    });
  });
});
